...
public class Main implements ModInitializer {
    ...
    public void onInitialize() {
        ServerWorldEvents.LOAD.register((server, world) -> {
            Path path = server.getSavePath(WorldSavePath.ROOT).resolve("tweakmoremore.json");
        ...
        ArgumentTypeRegistry.registerArgumentType(Identifier.of("tweakmoremore", "config_key"), ConfigKeyArgument.class, ConstantArgumentSerializer.of(ConfigKeyArgument::new));
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> ForceRandomTickCommand.register(dispatcher));
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment)
                                                       -> dispatcher.register(literal("rule")
                                                                                  .executes(ctx -> {
                                                                                      if (config.isEmpty()) {
                                                                                          ctx.getSource().sendFeedback(() -> Text.literal("No rules defined."), false);
                                                                                          return 1;
                                                                                      }

                                                                                      config.forEach((name, value) -> ctx.getSource().sendFeedback(() -> Text.literal("Rule[" + name + "]=" + value), false));

                                                                                      return 1;
                                                                                  })
                                                                                  .then(argument("name", ConfigKeyArgument.key())
                                                                                            ...
                                                                                            .then(argument("value", StringArgumentType.greedyString())
                                                                                                      .suggests((ctx, builder) -> {
                                                                                                          ...
                                                                                                          if (config.containsKey(name)) {
                                                                                                              ...
                                                                                                              if (value != null) {
                                                                                                                  builder.suggest(String.valueOf(value), Text.literal("current value"));
                                                                                                              }
                                                                                                          }
